#' @title  Summary  of Multi-trait Bayesian Models23
#' 
...
##------------------------------------------------------------------------------------------##
PostGibbs=function(local=getwd(),
                   burnIn=0,
                   thinning=1,
                   line=1,
                   HPD=.95,
    ...
    if(Summary==TRUE){
        ...
        if(length(grep("matrix",Matrix))==2){
            ...
            # Check if Genetic Matrix is diagonal
            isM1diag <- na.omit(n1matrix)[2] == 0
            ...
            # G = A + R          --> A and R are diagonal matrices (isM1diag = TRUE)
            #----------------------------------------------------------------------------------------#
            ...
            F=(n1matrix==traits & n2M==0 & !isM1diag)   ## Only additive matrix and diag(R)
            G=(n1matrix==traits & n2M==0 & isM1diag)   ## Only additive matrix and diag(R)
            if(is.na(F)){F=FALSE}
            #----------------------------------------------------------------------------------------#
            if(Uni==TRUE){
                #--------------------------------------------------------------------------------------#
